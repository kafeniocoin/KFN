...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Kafeniocoin client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Kafeniocoin"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Kafeniocoin"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
