...
const char * const PIVX_CONF_FILENAME = "kafeniocoin.conf";
const char * const PIVX_PID_FILENAME = "kafeniocoin.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Kafeniocoin only features
// Masternode
...
#else
    const char* pszModule = "kafeniocoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\kafeniocoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\kafeniocoin
// Mac: ~/Library/Application Support/kafeniocoin
// Unix: ~/.kafeniocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kafeniocoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\kafeniocoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\kafeniocoinParams
    // Mac: ~/Library/Application Support/kafeniocoinParams
    // Unix: ~/.kafeniocoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KafeniocoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "KafeniocoinParams";
#else
    // Unix
    return pathRet / ".kafeniocoin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty kafeniocoin.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Kafeniocoin config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override kafeniocoin.conf
