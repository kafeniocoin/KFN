...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected KFN and zKFN supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zKFNsupply']:
                assert_equal(node['zKFNsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check KFN and zKFN supply at the beginning
        # ------------------------------------------
        # zKFN supply: 2 coins for each denomination
        ...
        # KFN supply: block rewards minus burned fees for minting
        ...
        # Verify that KFN and zKFN supplies were properly updated after the spends and reorgs
        self.log.info("Check KFN and zKFN supply...")
