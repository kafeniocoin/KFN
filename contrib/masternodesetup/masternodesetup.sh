...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='kafeniocoin.conf'
CONFIGFOLDER='/root/.kafeniocoin'
COIN_DAEMON='kafeniocoind'
COIN_CLI='kafeniocoin-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ=`curl -s https://api.github.com/repos/KafeniocoinBF/Kafeniocoin/releases/latest | grep "browser_download_url.*Linux\\.tar.gz" | cut -d : -f 2,3 | tr -d \" | xargs`
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='kafeniocoin'
COIN_PORT=11427
RPC_PORT=11428
